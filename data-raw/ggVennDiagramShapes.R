...
f4e = tibble::tibble(
  shape_id = "401f",
  type = "ellipse",
  geometry = VennPlotData(setEdge = fancy_4d_ellipse(),  # how to store object in data.frame
                   setLabel = fancy_4d_ellipse_label()) %>% as.vector()
)
...
transform_venn_data = function(sets) {
  sets = sets %>%
    ...
    tidyr::nest(xy = c(x, y)) %>%
    dplyr::mutate(
      type = ifelse(v == 1, "ellipse", "polygon"),
      nsets = s,
...
sets = venn:::sets
sets =
  transform_venn_data(sets) |> dplyr::mutate(component = "setEdge")
...
scoords = data.frame(
  ...
  dplyr::ungroup()
scoords =
  transform_venn_data(scoords) |> dplyr::mutate(component = "setLabel")

ggVennDiagramShapes =
  dplyr::bind_rows(f4e, sets, scoords, f6t, f3c, f2c) %>%
